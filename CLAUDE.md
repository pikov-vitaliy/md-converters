...
- Python, на который указывают команды:
  ...
  `markitdown[pdf,docx,pptx,xlsx,xls,outlook]` ≥ 0.1. НЕ `[all]`: его
...
5. Слияние в универсальный `convert_to_md.py` (команда `tomd`), форматы
   ...
   секцией.
6. **Текущее состояние (v1.0.1):** чистка артефактов PPTX — битые
   картинки-заглушки сворачиваются в плейсхолдер, управляющие символы
   (`\x0b` и др.) вычищаются; подавлен pydub-warning про ffmpeg; переход
   с `markitdown[all]` (0.0.2 на Python 3.14) на формат-экстры (0.1.6) в
   pyproject/install.ps1.
