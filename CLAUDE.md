...
- **Известное ограничение:** примеры кода/конфигов из PDF НЕ оборачиваются
  в код-блоки (```), поэтому `<...>`-плейсхолдеры, ведущие `*`/`-` и
  отступы могут рендериться неидеально. Надёжное распознавание кода в PDF —
  отдельная задача (по желанию).
