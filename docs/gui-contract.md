...
- `note` — короткое пояснение для НЕ-ошибок (сейчас: почему файл
  пропущен при `status: skip`); показывается отдельной строкой под
  кнопками, лог в карточку не выводится;
