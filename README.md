...
- **Архивы** — загруженный `.zip` распаковывается: каждый файл
  внутри конвертируется отдельно, на выходе — один `.zip` с `.md`
  (структура сохранена), а не склейка содержимого в один файл.
- Настройки: front-matter, перезапись, картинки, PDF-таблицы,
  форматы; чекбокс **«URL: не проверять SSL»** (по умолчанию ВЫКЛ —
...
- Результаты — тремя выровненными областями (имя · кнопки · путь),
  путь по клику копируется. Скачивание по файлу или всё архивом.
