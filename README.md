...
интерфейсом (без окна консоли). Закрыли вкладку — сервер выключается
автоматически (примерно через минуту).

📖 **Полная справка по каждой кнопке и галочке** —
[`docs/gui-guide.md`](docs/gui-guide.md). А в самом интерфейсе наведите
курсор на любой пункт — всплывёт подсказка.

### Возможности

- **Drag-and-drop** файлов или ввод URL
- **С диска (на месте)** — кнопки «Выбрать файлы…/папку…» открывают
  родной диалог ОС и пишут `.md` **рядом с исходником** (папка —
  рекурсивно). Прямой путь «папку → `.md` на месте» из самого GUI,
  без ручного ввода путей.
- **Папка → ZIP** — выбрать папку целиком через браузер и получить
  один архив `.md` с сохранением структуры подпапок.
- **Архивы** — загруженный архив (`.zip`, `.7z`, `.tar`/`.tar.gz`/
  `.tar.bz2`/`.tar.xz`) распаковывается: каждый файл внутри
  конвертируется отдельно, на выходе — один `.zip` с `.md` (структура
  сохранена), а не склейка содержимого в один файл. RAR не
  поддерживается (нужен внешний проприетарный распаковщик).
- Настройки: front-matter, перезапись, картинки, PDF-таблицы,
  форматы; чекбокс **«Без проверки SSL»** (по умолчанию ВЫКЛ —
  для внутренних HTTPS с самоподписанным сертом; снимает защиту от
  MITM, включать только для доверенных хостов).
- **Превью** Markdown (marked.js + DOMPurify — защита от XSS);
  YAML-шапка прячется в сворачиваемый блок.
- Результаты — карточками: имя файла на всю ширину, под ним кнопки и
  путь (длинное имя не обрезается); путь по клику копируется.
  Скачивание по файлу или всё архивом.
- Настройки сохраняются между запусками (localStorage)
- Сервер слушает только `127.0.0.1`, проверяет Host и Origin (CSRF)

> Для пакетной конвертации папки «на месте» есть и не-GUI пути:
> правый клик по папке в Проводнике → «Конвертировать папку в
> Markdown» (после `install.ps1`), либо в консоли `tomd "C:\папка" -r`.
...
- **Картинки.** Встроенные картинки (base64) по умолчанию сворачиваются в
  ...
  также заменяются плейсхолдером. Флаг `--keep-images` оставляет
  встроенные картинки (HTML/DOCX) как есть, а **для PDF дополнительно
  извлекает растровые картинки постранично** (через `pypdfium2`) и
  дописывает их в конец `.md` секциями «Картинки страницы N»; без флага
  PDF-картинки не трогаются. С картинками `.md` самодостаточен, но
  заметно тяжелеет.
